...
program
  .name('onlyswaps-solver-v2')
  .description('TypeScript implementation of the OnlySwaps solver V2 (with conditions & intelligence)')
  .option(
    '-c, --config <path>',
  ...
  .option(
    '--use-v1',
    'Use Solver V1 instead of V2 (for comparison/testing)',
    false
  )
  .parse(process.argv);
...
async function main() {
  try {
    const solverVersion = options.useV1 ? 'V1' : 'V2';
    logger.info(`Starting OnlySwaps Solver ${solverVersion} (TypeScript)`);
    logger.info(`Features: ${options.useV1 ? 'Basic solver' : 'Conditional execution + Intelligent optimization'}`);
    ...
    // Start the app with SolverV2 (or V1 if --use-v1 flag is set)
    await App.start(networks, !options.useV1);
  } catch (error) {
